/*
 * Driver for Duolabs Dynamite Programmer SmartMouse/Phoenix readers
 *
...
static int dynamite_card_reset(ifd_reader_t *, int, void *, size_t);
static int dynamite_recv(ifd_reader_t *, unsigned int, unsigned char *, size_t, long);

/*
 * Common functions
 */
static int dynamite_setctrl(ifd_device_t *dev, const int  ctrl)
{
  int tmp;
 ...
 */
static int _dynamite_open(ifd_reader_t *reader, const char *device_name,
		      dynamite_priv_t *privd)
{
  ifd_device_params_t params;
  ifd_device_t *dev;

  privd->fd = open(DYNAMITE_DEVICE, O_RDWR);
  if (privd->fd < 0) {
     ct_error("error open %s", DYNAMITE_DEVICE);
  }

  if (privd->prot = prot_phoenix)
      if (ioctl(privd->fd, IOCTL_SET_PHOENIX_357) < 0)
          ct_error("error send IOCTL_SET_PHOENIX_357 command");

  if (privd->prot = prot_smartmouse)
      if (ioctl(privd->fd, IOCTL_SET_SMARTMOUSE_357) < 0)
          ct_error("error send IOCTL_SET_SMARTMOUSE_357 command");
...
static int dyn_phx_open(ifd_reader_t * reader, const char *device_name)
{
  dynamite_priv_t *privd = NULL;

  ifd_debug(1, "device=%s", device_name);
  reader->name = "Dynamite Phoenix Programmer";
  if (!(privd = (dynamite_priv_t *)malloc(sizeof (dynamite_priv_t))))
    {
  ...
  return _dynamite_open(reader, device_name, privd);
}

static int dyn_smtm_open(ifd_reader_t * reader, const char *device_name)
{
  dynamite_priv_t *privd = NULL;

  ifd_debug(1, "device=%s", device_name);
  reader->name = "Dynamite SmartMouse Programmer";
  if (!(privd = (dynamite_priv_t *)malloc(sizeof (dynamite_priv_t))))
    {
      ct_error("out of memory");
      return IFD_ERROR_NO_MEMORY;
 ...
 * Change the parity
 */
static int dynamite_change_parity(ifd_reader_t *reader, int parity)
{
 ...
 * Activate the reader
 */
static int dynamite_activate(ifd_reader_t * reader)
{
  ifd_device_t *dev = reader->device;
  dynamite_priv_t *privd = (dynamite_priv_t *)dev->user_data;
  int tmp;
  uint8_t mode;

  if (dynamite_card_reset(reader, 0, &mode, 1) < 0)
    return -1;
#if 0
...
#endif
  dynamite_change_parity(reader, tmp);

  return 0;
}

static int dynamite_deactivate(ifd_reader_t * reader)
{
  ifd_device_t *dev = reader->device;

  tcflush(dev->fd, TCIOFLUSH);
  if (dynamite_setctrl(dev, TIOCM_CTS))
    return -1;
  return 0;
}
 ...
 * Check card status
 */
static int dynamite_card_status(ifd_reader_t * reader, int slot, int *status)
{
  ifd_device_t *dev = reader->device;
  int tmp;
 ...
 */
static int dynamite_card_reset(ifd_reader_t *reader, int slot, void *atr,
			   size_t size)
{
  ifd_device_t *dev = reader->device;
  dynamite_priv_t *privd = dev->user_data;
  int res;

  if (slot)
    {
      ct_error("%s: bad slot index %u", 
	       (privd->prot == prot_phoenix) ? "dynamite_phoenix" : "dynamite_smartmouse", slot);
      return IFD_ERROR_INVALID_SLOT;
    }

  tcflush(dev->fd, TCIOFLUSH);
  if (dynamite_setctrl(dev, (privd->prot == prot_phoenix) 
  ...
  if ((res = dynamite_recv(reader, 0, (unsigned char *)atr, size, dev->timeout)) < 1)
    return -1;
 ...
 * Send command to IFD
 */
static int _dynamite_send(ifd_device_t *dev, const unsigned char *buffer, 
		      size_t len)
{
  unsigned char tmp;
  unsigned int i;
...
static int dynamite_send(ifd_reader_t * reader, unsigned int dad,
		     const unsigned char *buffer, size_t len)
{
  ...
  if (!dev)
    return -1;
  privd = (dynamite_priv_t *)dev->user_data;
  if (privd->mode == PHS_CONV_INDIRECT)
	...
	fbuff[i] = dir_conv_table[buffer[i]];
      i = _dynamite_send(dev, fbuff, len);
      free(fbuff);
      return i;
    }
  return _dynamite_send(dev, buffer, len);
}

/*
 * Receive data from IFD
 */
static int dynamite_recv(ifd_reader_t * reader, unsigned int dad,
 ...
 * Driver operations
 */
static struct ifd_driver_ops dyn_phx_driver;
static struct ifd_driver_ops dyn_smtm_driver;

void ifd_dynamite_register(void)
{
  dyn_phx_driver.open = dyn_phx_open;
  //dynamite_driver.change_parity = smph_change_parity;
  dyn_phx_driver.activate = dynamite_activate;
  dyn_phx_driver.deactivate = dynamite_deactivate;
  dyn_phx_driver.card_status = dynamite_card_status;
  dyn_phx_driver.card_reset = dynamite_card_reset;
  dyn_phx_driver.send = dynamite_send;
  dyn_phx_driver.recv = dynamite_recv;
  ifd_driver_register("dynamite_phoenix", &dyn_phx_driver);

  dyn_smtm_driver.open = dyn_smtm_open;
  //dynamite_driver.change_parity = smph_change_parity;
  dyn_smtm_driver.activate = dynamite_activate;
  dyn_smtm_driver.deactivate = dynamite_deactivate;
  dyn_smtm_driver.card_status = dynamite_card_status;
  dyn_smtm_driver.card_reset = dynamite_card_reset;
  dyn_smtm_driver.send = dynamite_send;
  dyn_smtm_driver.recv = dynamite_recv;
  ifd_driver_register("dynamite_smartmouse", &dyn_smtm_driver);
}
